...
# Cron: */30 * * * * /srv/dashboard/update-backup.sh >> /home/openclaw/.openclaw/logs/update-backup.log 2>&1

set -euo pipefail

GARAGE_TOKEN="${GARAGE_TOKEN:?GARAGE_TOKEN not set — add to ~/.openclaw/.env}"
GARAGE_BUCKET_ID="${GARAGE_BUCKET_ID:?GARAGE_BUCKET_ID not set — add to ~/.openclaw/.env}"
...
if lh_lines:
    # Column layout: NS NAME VOLUME SIZE SNAPSHOTCREATEDAT BACKUPTARGET STATE COMPLETIONTIME
    # Sort by completionTime (last column) to find most recent
    def lh_sort_key(line):
        parts = line.split()
        return parts[-1] if parts else ""
    lh_lines.sort(key=lh_sort_key)
    parts     = lh_lines[-1].split()
    lh_time   = parts[-1]  # completionTime is last field
