...
| Beelink GTi 13 Pro / px-0 (`10.57.57.254`) | Proxmox host — no K8s nodes. Runs Proxmox Datacenter Manager (links `pve`+`px-0` without a corosync cluster), `ollama` (AI alert-triage), `winserver` (stopped, AD lab). Also this repo's default DR-test target (see `DR.md`) | i9-13900H, 64GB, 2x2TB NVMe                 |
