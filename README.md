...
Nothing in the house is reachable from the internet. The public edge in
Frankfurt was retired on 2026-09-08 along with the public Jellyfin it served;
what is left of `edge_proxy` is a role with no host.

**One thing does not come from Flux, on purpose.** Nextcloud runs as VM 1000
under All-in-One, not in the cluster. AIO manages its sibling containers through
the Docker socket, which Kubernetes does not expose — the documented answer for
a Kubernetes environment is to run AIO in a VM. Moving it would mean leaving the
official installation method for a community chart that says it is for expert
use, hand-wiring Collabora, and trading AIO's application-aware Borg backup for
crash-consistent volume snapshots of a running Postgres. It is nine containers
behind one interface that updates and backs up itself; in the cluster it would
be eight manifests that do not. Evaluated 2026-09-09 and left alone.

The cost of the exception is a weekly `vzdump` image, watched by
`vzdump-freshness-check`, and a third irreplaceable secret — the Borg
passphrase, which lives in the password manager.
...
| n8n | default | One workflow: `daily-news-digest`, four RSS feeds into one Telegram message at 14:00. The four alerting workflows were removed 2026-09-09 — Alertmanager and Flux speak Telegram natively since 2026-09-07 |
...
| Borg passphrase | password manager | The 7.1 GB Nextcloud archive is unreadable. Generated by AIO and stored inside the VM it protects, so a lost VM loses it too |

None is recoverable from any backup — the restic password protects the only repo
that would hold a copy of it, and the Borg passphrase lives in the VM its
archive would restore. Keep a second copy somewhere that is not a password
manager.
